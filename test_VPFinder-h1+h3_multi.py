...
best_f1_path = "model_best/VPFinder-h1+h3_multi_f1.txt"
best_model_path = "model_best/VPFinder-h1+h3_multi_model.pth"
...
class MultiClassifier(nn.Module):
    ...
    def forward(self, input_id, comment_id, code_id, del_id, add_id, message_id, mask, comment_mask, code_mask, del_mask, add_mask, message_mask, vul_label_first_layer, vul_label_second_layer):
        ...
        comment_output = comment_output[:, 0, :]
        ...
        att_out3 = self.att3(solution_output, att_out1[0], att_out1[0])

        head_output = torch.mean(att_out3[0], dim=1)

        hidden_input = torch.cat((head_output, comment_output), dim=1)
