...
best_f1_path = "model_best/VulFixMiner_f1.txt"
best_model_path = "model_best/VulFixMiner_model.pth"
# 对按照CWE顶层关系分配样本后的数据集
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return batch_add, batch_del, batch_y
...
class VulClassifier(nn.Module):
    ...
    def forward(self, add_id, del_id, add_mask, del_mask):
...
def get_model_output(model, val_data):
    ...
    try:
        ...
        print("Best model not found. Please complete the binary classification model training first!")
    ...
    with torch.no_grad():
        for adds, dels, labels in tqdm(val_dataloader):
            ...
            outputs = model(add_ids, del_ids, add_mask, del_mask)
