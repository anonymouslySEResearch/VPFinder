...
best_f1_path = "model_best/VPFinder-p_binary_f1.txt"
best_model_path = "model_best/VPFinder-p_binary_model.pth"
...
class MyDataset(Dataset):
    ...
    def get_batch_del_samples(self, idx):
        # Fetch a batch of texts
        return self.patch_del[idx]

    def get_batch_add_samples(self, idx):
        # Fetch a batch of texts
        return self.patch_add[idx]

    def __getitem__(self, idx):
        batch_del_samples = self.get_batch_del_samples(idx)
        batch_add_samples = self.get_batch_add_samples(idx)
        batch_y = self.get_batch_labels(idx)
        return batch_del_samples, batch_add_samples, batch_y
...
class VulClassifier(nn.Module):
    ...
    def forward(self, del_sample_id, add_sample_id, del_sample_mask, add_sample_mask):
        pooled_output_del = self.bert1.roberta(input_ids=del_sample_id, attention_mask=del_sample_mask, return_dict=False)[0]
        pooled_output_add = self.bert1.roberta(input_ids=add_sample_id, attention_mask=add_sample_mask, return_dict=False)[0]
        del_output = pooled_output_del[:, 0, :]
        add_output = pooled_output_add[:, 0, :]
        input = torch.cat((del_output, add_output), dim=1)
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch in range(epochs):
        ...
        for del_samples, add_samples, labels in tqdm(train_dataloader):
            del_ids = del_samples['input_ids'].squeeze(1).to(device)
            del_mask = del_samples['attention_mask'].squeeze(1).to(device)
            add_ids = add_samples['input_ids'].squeeze(1).to(device)
            add_mask = add_samples['attention_mask'].squeeze(1).to(device)
            labels = labels.long().to(device)

            outputs = model(del_ids, add_ids, del_mask, add_mask)
        ...
        with torch.no_grad():
            for del_samples, add_samples, labels in tqdm(val_dataloader):
                del_ids = del_samples['input_ids'].squeeze(1).to(device)
                del_mask = del_samples['attention_mask'].squeeze(1).to(device)
                add_ids = add_samples['input_ids'].squeeze(1).to(device)
                add_mask = add_samples['attention_mask'].squeeze(1).to(device)
