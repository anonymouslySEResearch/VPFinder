...
best_f1_path = "model_best/VPFinder-ID_multi_f1.txt"
best_model_path = "model_best/VPFinder-ID_multi_model.pth"
...
class MultiClassifier(nn.Module):
    ...
    def forward(self, input_id, comment_id, code_id, mask, comment_mask, code_mask, vul_label_first_layer, vul_label_second_layer):
        ...
        hidden_input = torch.mean(description_output, dim=1)
...
def train_multi_model(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch in range(epochs):
        ...
        for inputs, comments, codes, labels in tqdm(train_dataloader):
            ...
            outputs, preds = model(input_ids, comment_ids, code_ids, attention_mask, comment_mask, code_mask, vul_label_first_layer, vul_label_second_layer)
        ...
        with torch.no_grad():
            for inputs, comments, codes, labels in tqdm(val_dataloader):
                input_ids = inputs['input_ids'].squeeze(1).to(device)
                comment_ids = comments['input_ids'].squeeze(1).to(device)
                code_ids = codes['input_ids'].squeeze(1).to(device)
                attention_mask = inputs['attention_mask'].squeeze(1).to(device)
