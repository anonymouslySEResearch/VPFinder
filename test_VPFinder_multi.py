...
best_f1_path = "model_best/VPFinder_multi_f1.txt"
best_model_path = "model_best/VPFinder_multi_model.pth"
...
class MultiClassifier(nn.Module):
    ...
    def forward(self, input_id, comment_id, code_id, del_id, add_id, message_id, mask, comment_mask, code_mask, del_mask, add_mask, message_mask, vul_label_first_layer, vul_label_second_layer):
        ...
        # 最后一层隐藏层状态
        text_output = text_output[:, 1:, :]   # [20, 511, 768]
        comment_output = comment_output[:, 1:, :]
        # 提取 [CLS] 标记的输出
        code_output = code_output[:, 0, :]   # [20, 768]
        code_output = code_output.unsqueeze(1)   # [20, 1, 768]
        code_output = code_output.repeat(1, 511, 1)   # [20, 511, 768]

        att_out1 = self.att1(code_output, text_output, text_output)   # [20, 511, 768]  [20, 511, 768]  [20, 511, 768]
        att_out2 = self.att2(code_output, comment_output, comment_output)  # [20, 511, 768]  [20, 511, 768]  [20, 511, 768]
        description_output = torch.cat((att_out1[0], att_out2[0]), dim=2)  # [20, 511, 768 * 2]

        del_output = del_output[:, 0, :]
        del_output = self.patch_del_hidden(del_output)
        add_output = add_output[:, 0, :]   # [20, 768]
        add_output = self.patch_add_hidden(add_output)   # [20, 384]
        message_output = message_output[:, 0, :]
        solution_output = torch.cat((message_output, del_output, add_output), dim=1)   # [20, 768 * 2]
        solution_output = solution_output.unsqueeze(1)
        solution_output = solution_output.repeat(1, 511, 1)

        att_out3 = self.att3(solution_output, description_output, description_output)   # [20, 511, 768 * 2]
        hidden_input = torch.mean(att_out3[0], dim=1)  # [20, 768 * 2]

        hidden_output = self.hidden(hidden_input)  # [20, 64]

        mapped_label_info_first = self.label_info(vul_label_first_layer)

        label_info_first = mapped_label_info_first.squeeze(dim=1)  # [8, 64]

        mapped_label_info_second = self.label_info_second(vul_label_second_layer)

        hidden_output = hidden_output.unsqueeze(1)

        label_info_first = label_info_first.unsqueeze(0)  # [1, 8, 64]
        label_info_first = label_info_first.repeat(hidden_output.shape[0], 1, 1)  # [20, 8, 64]
...
def train_multi_model(model, val_data):
    ...
    with torch.no_grad():
        for inputs, comments, codes, delete, add, messages, labels in tqdm(val_dataloader):
            ...
            outputs, preds = model(input_ids, comment_ids, code_ids, del_ids, add_ids, message_ids, attention_mask, comment_mask, code_mask, del_mask, add_mask, message_mask, vul_label_first_layer, vul_label_second_layer)
